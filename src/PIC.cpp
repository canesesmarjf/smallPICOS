...
// #########################################################################################################
/*
void PIC_TYP::interpEM(const params_TYP * params, const mesh_TYP * mesh, const ions_TYP * IONS, const fields_TYP * fields, arma::rowvec * ZN, arma::rowvec * EM)
{
    // Assign cell:
    double xp   = (*ZN)(0);
...
// #########################################################################################################
/*
void PIC_TYP::advanceParticles(const params_TYP * params, const mesh_TYP * mesh, fields_TYP * fields, vector<ions_TYP> * IONS)
{
...
void PIC_TYP::assignCell(const params_TYP * params, const mesh_TYP * mesh, ions_TYP * IONS)
{
  ...
  for(int ii=0; ii<N_CP; ii++)
  {
		...
		// Check boundaries:
		// When particles reach the boundaries, this makes sure charge is not
		// lost to the ghost cells during the extrapolation of moments.
		// This happens becuase the shape function spreads the particle's charge
		// contribution to 3 adjacent cells.
		if (m == params->mesh_params.Nx - 1)
		{
			IONS->wxc(ii) = IONS->wxc(ii) + IONS->wxr(ii);
			IONS->wxr(ii) = 0;
		}
		else if (m == 0)
		{
			IONS->wxc(ii) = IONS->wxc(ii) + IONS->wxl(ii);
			IONS->wxl(ii) = 0;
		}
...
void PIC_TYP::eim(const params_TYP * params, fields_TYP * fields, ions_TYP * IONS)
{
	...
	IONS->n_m   *= (1/A)*IONS->K/params->mesh_params.dx;
	IONS->nv_m  *= (1/A)*IONS->K/params->mesh_params.dx;
	IONS->P11_m *= (1/A)*IONS->K/params->mesh_params.dx;
	IONS->P22_m *= (1/A)*IONS->K/params->mesh_params.dx;
...
void PIC_TYP::calculateDerivedIonMoments(const params_TYP * params, ions_TYP * IONS)
{
	...
	// Ion temperatures:
	IONS->Tpar_m = Ppar/(F_E*IONS->n_m);
	IONS->Tper_m = Pper/(F_E*IONS->n_m);
}
